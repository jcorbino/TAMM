...
#endif
#include "tamm/cpu_memory_pool.hpp"
...
void allocate_host_buffers(ExecutionHW hw, T*& host_buf, size_t buf_size) {
  ...
  // }
  // else {
  auto& memPool = tamm::CPUPooledStorageManager::getInstance();
...
bool transpose_inputs(ExecutionHW hw, gpuStream_t& thandle, T2* ainter_buf,
  ...
  if(hw == ExecutionHW::GPU) {
    ...
    // copy_data_to_gpu(hw, thandle, abuf, asize, ainter_buf_dev_in, bbuf, bsize,
    // binter_buf_dev_in);
    ...
    // gpuMemcpyAsync<T2>(ainter_buf_dev, ainter_buf_dev_in, asize, gpuMemcpyDeviceToDevice,
    // thandle); gpuMemcpyAsync<T3>(binter_buf_dev, binter_buf_dev_in, bsize,
    // gpuMemcpyDeviceToDevice, thandle);

    free_device_buffers(hw, ainter_buf_dev, asize);
    free_device_buffers(hw, binter_buf_dev, bsize);

    ainter_buf_dev = ainter_buf_dev_in;
    binter_buf_dev = binter_buf_dev_in;
